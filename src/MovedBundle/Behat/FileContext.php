...
class FileContext extends DefaultContext
{
    ...
    public function createFileList(TableNode $tableNode)
    {
        ...
        foreach ($tableNode as $hash) {
            ...
            $user = $this->getEntityManager()->getRepository('MovedBundle:User')->findOneByUsername($hash['username']);
            $userWithAccess = $this->getEntityManager()->getRepository('MovedBundle:User')->findOneByUsername($hash['userWithAccess']);
            $folder = $this->getEntityManager()->getRepository('MovedBundle:Folder')->findOneByName($hash['folder']);
            $tag = $this->getEntityManager()->getRepository('MovedBundle:Tag')->findOneByName($hash['tags']);
    ...
    public function createFolderList(TableNode $tableNode)
    {
        ...
        foreach ($tableNode as $hash) {
            ...
            foreach ($tags as $tag) {
                $entity = $this->getEntityManager()->getRepository('MovedBundle:Tag')->findOneByName(trim($tag));
    ...
    {
        $user = $this->getEntityManager()->getRepository('MovedBundle:User')->findOneByUsername($username);
        /** @var Folder $folder */
        $folder = $this->getEntityManager()->getRepository('MovedBundle:Folder')->findOneByName($folderName);
    ...
    public function hasAccessToFile($username, $filename)
    {
        $user = $this->getEntityManager()->getRepository('MovedBundle:User')->findOneByUsername($username);
        /** @var File $file */
        $file = $this->getEntityManager()->getRepository('MovedBundle:File')->findOneByName($filename);
